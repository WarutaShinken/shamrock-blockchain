...
class FullNode:
    ...
    def set_server(self, server: ShamrockServer):
        ...
        elif self.config["port"] == 9359:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("introducer-testnet.shamrock.network")
        try:
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSShamrockConnection]
    ):
        """
