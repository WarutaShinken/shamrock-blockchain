...
from shamrock.ssl.create_ssl import generate_ca_signed_cert, get_shamrock_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    if "srn_target_address" not in config["farmer"]:
        print(f"Setting the srn destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "srn_target_address" not in config["pool"]:
        print(f"Setting the srn destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    shamrock_ca_crt_path.write_bytes(shamrock_ca_crt)
    shamrock_ca_key_path.write_bytes(shamrock_ca_key)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if shamrock_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return shamrock_init(root_path)
...
def shamrock_init(root_path: Path):
